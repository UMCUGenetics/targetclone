...
class Simulator:
	...
	#Mixing fractions contains the tumor fractions	
	...
	def sampleRandomCMu(self, length):
		...
		for pos in range(0, length):
			...
			kValues = range(self.kmin, self.kmax+1)
	...
	def subclonalExpansion(self):
		...
		for iteration in range(0,self.simulationRounds):
			...
			somVarArray = []
			for variant in preGCNIS.somaticVariants: #I don't think pre-GCNIS ever has somatic variants, so this can just be 0
					somVarArray.append(0)
			...
			##Only if the simulation is random restarts: initialize with random values
			if simulationSettings.general['randomTree'] == True:
				randomCMu = self.sampleRandomCMu(len(measurements[1].measurements))
				preGCNISSample.bestCMu = randomCMu
				preGCNISSample.originalCMu = randomCMu
				preGCNISSample.A = self.sampleRandomA(randomCMu)
			...
			if simulationSettings.general['randomTree'] == True:
				randomCMu = self.sampleRandomCMu(len(measurements[1].measurements))
				GCNISSample.bestCMu = randomCMu
				GCNISSample.originalCMu = randomCMu
				GCNISSample.A = self.sampleRandomA(randomCMu)
			...
			somVarArray = self.generateMixedSampleSNVMeasurements([GCNIS], [100-self.muN])
			...
			for clone in clones: #make a new sample for each clone
				...
				# 	somVarArray.append(variant.value)
				# 	# if variant.value == 1:
				# 	# 	somVarArray.append(1)
				# 	# else:
				# 	# 	somVarArray.append(0)
				somVarArray = self.generateMixedSampleSNVMeasurements([clone], [100-self.muN])
			...
			for cloneInd in range(0, len(finalClones)):
				...
				for variant in range(0, len(somVar)):
					somVarMatrix[variant][cloneInd] = somVar[variant].value
			...
			#if we use a separate SNV matrix for output, TC will use the actual binary values when running. 
			vafSNVMatrix = np.empty([len(samples[0].somaticVariants), (len(samples)-1)], dtype=float)
			for sampleInd in range(1, len(samples)):
				somVar = samples[sampleInd].somaticVariants
				for variant in range(0, len(somVar)):
					vafSNVMatrix[variant][sampleInd-1] = somVar[variant]
			
			#Somatic variants matrix
			np.savetxt(newDir + '/SomVar_' + str(iteration) + '.txt', vafSNVMatrix, fmt='%f', delimiter='\t')
			
			#In these simulations I will also store the VAF matrix in the storage
			simulationDataHandler = SimulationDataHandler()
			simulationDataHandler.setSimulationData(cMatrix, aMatrix, samples, measurementsMatrix, vafSNVMatrix, realTree, self.chromosomes, self.positions)
