...
class Simulator:
	...
	numberOfSVs = simulationSettings.general['numberOfSVs']
	numberOfArmChanges = simulationSettings.general['numberOfArmChanges']
	numberOfAllelicAmps = simulationSettings.general['numberOfAllelicAmps'] ##Not used because it generates too much noise!!!
	
	#Define the number of changes that can happen during each simulated cell cycle
	malignantWcLosses = simulationSettings.general['malignantWcLosses']
	malignantArmLosses = simulationSettings.general['malignantArmLosses']
	malignant12pCopies = simulationSettings.general['malignant12pCopies']
	malignantSVGains = simulationSettings.general['malignantSVGains']
	
	cycleSVGains = simulationSettings.general['cycleSVGains']
	cycleArmGains = simulationSettings.general['cycleArmGains']
	cycleArmLosses = simulationSettings.general['cycleArmLosses']
	...
	simulationProbabilityFile = simulationSettings.files['simulationProbabilityFile'] #make all these things a separate settings file later
	numberOfSamples = 10 #NOt used, but could potentially be introduced back into the model to limit the number of samples (cell cycles does the same, but in a less controlled fasion)
	
	permutationRounds = 1 #this parameter is obsolete
	simulationRounds = 1
	...
	noiseLevel = float(simulationSettings.general['noiseLevel'])
	segmentation = None
	#These are provided by the bash script
	muN = 0
	uniqueID = 0
	
	def initialize(self, muN, uniqueID):
	...
	def parseReferenceFile(self):
		...
		print "reference: ", self.referenceFile
		print os.getcwd()
		with open(self.referenceFile, "r") as inFile:
			...
			for line in inFile:
				...
				chromosomeArmName = self.segmentation.getSegmentName(chromosome, position, position)
	...
	def generateMixedSampleSNVMeasurements(self, clones, mixingFractions):
		...
		for variant in range(0, variants):
			...
			for cloneInd in range(0, len(clones)):
				...
				allele = clone.A[self.variantPositions[variant]].getAllelesAsString()
		...
		# print len(clone.A)
		# #print clone.somaticVariants
		# for variant in range(0, len(clone.somaticVariants)):
		# 	print clone.somaticVariants[variant].ind
		# 	#print self.variantPositions[variant]
		# 	print clone.A[self.variantPositions[variant]].getAllelesAsString()
		# 	print clone.somaticVariants[variant].alleles
		# 	print clone.somaticVariants[variant].value
		# 
		# 
		# #Per clone, check if any of the clones has a somatic SNV. If yes, collapse into 1 for that sample
		# sampleSomaticVariants = [0]*len(clones[0].somaticVariants)
		# for clone in clones:
		# 	cloneSNVs = clone.somaticVariants
		# 	for variantInd in range(0, len(cloneSNVs)):
		# 		if cloneSNVs[variantInd].value == 1:
		# 			sampleSomaticVariants[variantInd] = 1
		# 
		# return sampleSomaticVariants
	...
	def subclonalExpansion(self):
		...
		#Load the targetClone instance from a pickled file, make sure that we load the right one!
		...
		for iteration in range(0,self.simulationRounds):
			...
			healthySubclone.C = [C([2,2])]*(self.numberOfMeasurements)
			...
			precursor4N.C = [C([2,4])]*(self.numberOfMeasurements)
			...
			#Make the precursor sample separately, only used in the first iteration of the method for the star tree
			...
			#obtain the chromosome, start and end information from the other samples
			measurements = self.generateMeasurements(precursor4N, 100) #the mu of the tumor is here 0 because we have a healthy sample
			...
			eventDistances = EventDistances(self.kmin, self.kmax)
			bestCMu4N = CMuCombination(C([2,4]), Mu(0), eventDistances)
			...
			#Should be sufficient to capture the names and relationships for the ground truth tree
			preGCNIS.parent = precursor4N
			GCNIS.parent = preGCNIS
			
			
			allClones = [preGCNIS, GCNIS] #here we store all clones that we have made
			clones = self.evolve(GCNIS, 1, 1, simulationProbabilities, [])
			
			clonesToSample = [preGCNIS, GCNIS]
			clonesToSample += clones
			finalClones = [precursor4N, preGCNIS, GCNIS]
			finalClones += clones
			
			#Mix clones in samples
			sampleNum = len(clonesToSample)
			print "number of samples to add"
			
			edgeList = [] #edges of the ground truth tree
			
			#First, select a clone to mix with given the number of mixing clones
			#Select random mixing fractions
			#Generate measurements
			#Create SV measurements
			#Select a tumor/normal fraction (the Mu object currently supports only tumor/normal)
			#Make sample object
			samples = [precursor4NSample]
			savedMu = [Mu(0)]
			sampleNames = ["precursor"]
			for sample in range(0, sampleNum):
				
				#Obtain the main clone information
				clone = clonesToSample[sample]
				print "processing clone: ", clone.name

				
				
				possibleSamplesToMix = range(0,sample) + range(sample+1,sampleNum) #the samples to select can never be the sample itself
				print "can mix with: ", possibleSamplesToMix
				
				subclonesToMix = random.sample(possibleSamplesToMix, self.numberOfMixedClones) #Select x other samples to mix the current sample with
				...
				for subcloneInd in subclonesToMix:
					selectedClones.append(clonesToSample[subcloneInd])
				...
				print self.maximumMinorCloneFrequency,self.numberOfMixedClones+1,self.maximumMinorCloneFrequency
				...
				while totalMinorFraction < self.minimumMinorCloneFrequency:
				
					#sum to equal, N to sample, maxval, minval
					minorCloneFractionsFull = randvec(self.maximumMinorCloneFrequency,self.numberOfMixedClones+1,self.maximumMinorCloneFrequency,1) #Sample the minor clone fractions
					
					print "minor fractions: ", minorCloneFractionsFull
					minorCloneFractions = random.sample(minorCloneFractionsFull, self.numberOfMixedClones)
			...
			#generate the ground truth C, A, SNVs, mu and T
			...
			somVarMatrix = np.empty([len(samples[0].somaticVariants), len(samples)], dtype=float)
			for cloneInd in range(0, len(finalClones)):
				somVar = samples[cloneInd].somaticVariants
			...
			print "total error without ambiguities: ", totalScore
		...
		# 		pAmbiguityScores.append(ambiguityScore)
		# 		pAmbiguityAllScores.append(totalScore)
		...
		# 	allPAmbiguityScores.append(sum(pAmbiguityScores) / len(pCScores))
		# 	allPAmbiguityCorrectedScores.append(sum(pAmbiguityAllScores) / len(pCScores))
		# 	
		# 	
		...
		# f.close()
		# f = open(newDir + '/pAmbiguityError.txt', 'w')
		# f.write(','.join([str(i) for i in allPAmbiguityScores]))  # python will convert \n to os.linesep
		# f.close()
		# f = open(newDir + '/pAmbiguityCorrectedError.txt', 'w')
		# f.write(','.join([str(i) for i in allPAmbiguityCorrectedScores]))  # python will convert \n to os.linesep
		# f.close()
		# 
	...
	def permutations(self, afMatrix, somVarMatrix, samples):
		...
		for permutation in range(0, self.permutationRounds):
			p = Process(target=permutationHandler.permute, args=(q,permutation))
			jobList.append(p)
			p.start()
		...
		# p1 = Process(target=permutationHandler.permute, args =(q,"1"))
		# p2 = Process(target=permutationHandler.permute, args =(q, "2"))
		# 
		# p1.start()
		# p2.start()
		# 
		...
		for permutation in range(0,self.permutationRounds):
			pool.apply_async(permutationHandler.permute, permutation)
	...
	#Somehow we sometimes end up missing certain clones that end up having no parent in the tree. 	
			...
			if subclone.name == 'GCNIS': #if the parent is GCNIS, we assume that we are unlimited in our sources and we can keep searching until we find a viable subclone. 
				...
				while passed == False:
					...
					if newSubclone != False:
						...
						passed = True
						clones.append(newSubclone) #I think when we end up here, the new subclone was never added to the clones. Only its children are gtting added and that's why it's weird. 
			else:
				return clones
			
		else:
			#clones.append(subclone)
			#I think maybe a certain sample does not get appended in some cases, even though its children do. 
	...
	def generateMeasurements(self, sample, randomMuT):
		...
		#currentNoiseLevel = 0
		currentNoiseLevel = self.noiseLevel
