...
class Simulator:
	...
	def initialize(self, muN, uniqueID, leaveOut):
		self.muN = muN
		self.uniqueID = uniqueID
		self.leaveOut = int(leaveOut)
		#Pre-processing:
	...
	def subclonalExpansion(self):
		...
		for iteration in range(0,self.simulationRounds):
			...
			measurements = self.generateMeasurements(preGCNIS, self.muN) #assume that this is the last precursor we know, which is then 100% tumor. 
			...
			randomMuT = random.sample(range(0, 100), 1)[0]#mu of the tumor component
			#Calculate these measurements from the counts
			measurements = self.generateMeasurements(GCNIS, 100-self.muN)
			...
			GCNISSample.Mu = Mu(self.muN)
			
			
			
			#Make sure that we can always leave out enough clones
			print "leaving out: ", self.leaveOut
			cloneLen = -1
			while cloneLen <= self.leaveOut + 1: #Make sure that there are at least always 2 samples to avoid later divisions by zero when checking if 80% of the tree is present, where we exclude the healthy sample
				clones = self.evolve(GCNIS, 1, 1, simulationProbabilities, [])
				cloneLen = len(clones) + 3 #we always have at least 3, these are the precursors.
				print "number of clones: ", cloneLen
			
			savedMu = [precursor4NSample.Mu, preGCNISSample.Mu, GCNISSample.Mu]
			cloneInd = 0
			
			#we wish to randomly leave certain clones out given the leaveOut number.
			#select a random number from the total clones (4N precursor does not count)
			#do not take a sample of this clone
			
			
			allClones = [precursor4N, preGCNIS, GCNIS]
			allClonesUnfiltered = allClones + clones
			for clone in clones:
				print clone.name
			#Select random clones to leave out
			selectedClones = random.sample(range(1, len(clones)+3), self.leaveOut)
			print selectedClones
			#Check if any of the precursor samples have been selected for removal
			finalClones = [precursor4N, preGCNIS, GCNIS]
			samples = [precursor4NSample, preGCNISSample, GCNISSample]
			
			selectedCloneNames = []
			
			for clone in selectedClones:
				selectedCloneNames.append(allClonesUnfiltered[clone].name)
			print selectedCloneNames
			#If we remove a sample, we should also remove its links. Maybe it is easier to do that after the tree is made? 
			...
			#For every edge in the edge list, check if the sample is part of the list to be removed (use sample names here)
			#if true, search through the edge list and update the parent to the parent of the parent.
			newEdgeList = []
			for edge in edgeList:
				
				currentParent = edge[1]
				
				#if the parent matches a removed node, update it
				if edge[1] in selectedCloneNames:
					
					parentFound = False
					while parentFound is False:
						if currentParent not in selectedCloneNames:
							parentFound = True
						else:
							for edge2 in edgeList:
								if edge2[2] == currentParent:
									currentParent = edge2[1]
									
		
				#update to the new parent.
				if edge[2] not in selectedCloneNames:
					
					newEdge = (0, currentParent, edge[2])
					
					newEdgeList.append(newEdge)
			print newEdgeList
			edgeList = newEdgeList
			#Also update the samples, remove the ones that we leave out
			updatedSamples = []
			updatedClones = []
			for sample in range(0, len(samples)):
				if sample not in selectedClones:
					updatedSamples.append(samples[sample])
			for clone in range(0, len(finalClones)): #the 4n precursor is not part of the clones, so skip it here
				if clone not in selectedClones:
					updatedClones.append(finalClones[clone]) #not working? Check
			
			for clone in updatedClones:
				print clone.C[0].c
			
			samples = updatedSamples
			finalClones = updatedClones
			for sample in range(0, len(samples)):
			...
			#write to a file how many clones we left out
			f = open(newDir + '/leftOut.txt', 'w')
			f.write(str(self.leaveOut))  # python will convert \n to os.linesep
			f.close()
